...
var getTotalDaysInMonth = require('../utils/getTotalDaysInMonth');
...
const context$2 = React__namespace.createContext(null);
...
as: Component = 'ul', popperConfig, variant, isNav, ...props }, ref) => {
    ...
    const isInputGroup = React.useContext(context$2);
    const alignClasses = [];
    if (align) {
...
const Calendar = React__namespace.forwardRef((props, ref) => {
    const { focusedDateIndex, setFocusedDateIndex, focusedYearIndex, setYearPositionIndex, } = React__namespace.useContext(context);
    const handleClick = (e) => {
    ...
    for (let i = 0; i < 9; i++) {
        ...
        for (let j = 0; j <= 6; j++) {
            if (day <= monthLength && (i > 0 || j >= startingDay)) {
                ...
                const localizedDate = dayjs__default["default"](date).format('dddd, MMMM D, YYYY');
                ...
                week.push(React__namespace.createElement("td", { key: j, role: "button", "aria-label": localizedDate, "aria-selected": hasSelectedDate ? 'true' : undefined, "aria-current": isCurrentDate ? 'date' : undefined, "data-day": day, onClick: isOutOfMinMaxRange ? undefined : handleClick, className: classNames__default["default"](isCurrentDate && 'text-primary', isOutOfMinMaxRange && 'disabled', hasSelectedDateInModeSingle && 'text-white bg-primary-600', areSelectedEnds && 'bg-primary-600 text-white', notSelectedEnds && 'bg-primary-100'), tabIndex: -1, ref: (el) => props.dayRefs.current
    ...
    const handleKeyDown = (event) => {
        ...
        const handleDateIncrement = (incrementedDay) => {
            const totalDaysInMonth = getTotalDaysInMonth.getTotalDaysInMonth(props.displayDate);
    ...
    return (React__namespace.createElement("table", { className: "text-center", role: "grid", ref: ref, onKeyDown: handleKeyDown },
...
const YearView = React__namespace.forwardRef(({ selectedDate, displayDate, onClickYear, show, yearRefs, onChangeMonth, handleTabPressOnCalendarBody, ...props }, ref) => {
    const { focusedYearIndex, setFocusedYearIndex } = React__namespace.useContext(context);
    ...
    return (React__namespace.createElement("div", { className: "sgds yearpicker", ref: ref, ...props }, yearArray.map((year, index) => {
        const activeYearClass = getActiveYearClass(year);
        const isCurrentYear = CURRENT_YEAR === year;
        return (React__namespace.createElement("button", { type: "button", "aria-selected": activeYearClass ? 'true' : 'false', "aria-label": isCurrentYear ? `Current year, ${year}` : undefined, className: classNames__default["default"](isCurrentYear && 'text-primary', activeYearClass, 'year'), key: year, onClick: () => onClickYear(year), onKeyDown: handleKeyDown, ref: (el) => yearRefs.current ? (yearRefs.current[index] = el) : undefined, disabled: year < 1900 ? true : false }, year));
    })));
});
...
const CalendarHeader = React__namespace.forwardRef((props, ref) => {
    const { view, setView } = React__namespace.useContext(context);
    ...
    const renderPreviousButton = () => {
        ...
        const previousButton = (React__namespace.createElement("button", { type: "button", onClick: handleClickPrevious, onKeyDown: handlePressPrevious, "aria-label": ariaLabels[view] },
        ...
        switch (view) {
            case 'day':
                if (props.displayDate.getDate() === startLimitDate.getDate() &&
                    ...
                    return React__namespace.createElement("div", { style: { width: 32 } });
                }
    ...
    const renderNextButton = () => {
        ...
        return (React__namespace.createElement("button", { type: "button", onClick: handleClickNext, onKeyDown: handlePressNext, "aria-label": ariaLabels[view] },
    ...
    const renderHeader = () => {
        ...
        return (React__namespace.createElement("button", { type: "button", onClick: changeView, onKeyDown: handlePressChangeView, "aria-disabled": view === 'year', className: view === 'year' ? "disabled" : undefined, "aria-live": "polite", "aria-label": ariaLabel }, header));
    };
    return (React__namespace.createElement("div", { className: "text-center d-flex justify-content-between", ref: ref },
...
const DateInput = React__namespace.forwardRef(({ className, 
// Need to define the default "as" during prop destructuring to be compatible with styled-components github.com/react-bootstrap/react-bootstrap/issues/3595
as: Component = FormControl$1, required, placeholder, disabled, id, isRange, inputDate, isInvalid, dateFormat, validateDateInput, enterDateRange, enterDateSingle, ...props }, ref) => {
    ...
    return (React__namespace.createElement(InputMask__default["default"], { ...controlProps, mask: maskedDateFormat, alwaysShowMask: true, maskPlaceholder: defaultPlaceHolder, "aria-label": "Enter Date", onChange: isRange ? enterDateRange : enterDateSingle, onBlurCapture: validateDateInput, ref: inputRef },
        React__namespace.createElement(Component, { className: classNames__default["default"](className), ref: inputRef, ...props })));
});
...
const MonthView = React__namespace.forwardRef(({ selectedDate, displayDate, onClickMonth, show, monthRefs, onChangeMonth, handleTabPressOnCalendarBody, ...props }, ref) => {
    const { focusedMonthIndex, setFocusedMonthIndex } = React__namespace.useContext(context);
    const handleKeyDown = (event) => {
        event.preventDefault();
    ...
    return (React__namespace.createElement("div", { className: "sgds monthpicker", ref: ref, ...props }, MONTH_LABELS.map((month, index) => {
        ...
        return (React__namespace.createElement("button", { type: "button", "aria-label": isCurrentMonthAndYear ? currentMonthAriaLabel : defaultAriaLabel, "aria-selected": activeMonthClass ? 'true' : 'false', className: classNames__default["default"](isCurrentMonthAndYear && 'text-primary', activeMonthClass, 'month'), key: month, onClick: () => onClickMonth(index), onKeyDown: handleKeyDown, ref: (el) => monthRefs.current ? (monthRefs.current[index] = el) : undefined }, month.slice(0, 3)));
    })));
});
...
const propTypes = {
    ...
    invalidFeedback: PropTypes__default["default"].string,
    disabled: PropTypes__default["default"].bool,
...
const isValidDate = (date, dateFormat) => {
    return dayjs__default["default"](date, dateFormat, true).isValid();
};
...
const DatePicker = React__namespace.forwardRef(({ dateFormat = 'DD/MM/YYYY', calendarPlacement = 'down', mode = 'single', displayDate = new Date(), flip = true, clearBtnVariant = 'primary', ...props }, ref) => {
    ...
    const dropdownToggleRef = React__namespace.useRef(null);
    const calendarHeaderRef = React__namespace.useRef(null);
    const dayRefs = React__namespace.useRef([]);
    const monthRefs = React__namespace.useRef([]);
    const yearRefs = React__namespace.useRef([]);
    const getinitialInputDate = () => {
        ...
        if (!isRange && props.initialValue instanceof Date) {
            return dayjs__default["default"](props.initialValue).format(dateFormat);
        }
        return dateFormat.toLowerCase();
    };
    const initialState = {
    ...
    const [datepickerMenuId, setDatepickerMenuId] = React__namespace.useState('');
    const [state, setState] = React__namespace.useState(initialState);
    const [view, setView] = React__namespace.useState('day');
    const [focusedDateIndex, setFocusedDateIndex] = React__namespace.useState(0);
    const [focusedMonthIndex, setFocusedMonthIndex] = React__namespace.useState(0);
    const [focusedYearIndex, setFocusedYearIndex] = React__namespace.useState(0);
    const [yearPositionIndex, setYearPositionIndex] = React__namespace.useState(0);
    ...
    const updateFocusedDate = React__namespace.useCallback((focusedDate) => {
    ...
    const onChangeDateSingle = (newSelectedDate) => {
        setState((prevState) => ({
            ...prevState,
            inputDate: dayjs__default["default"](newSelectedDate).format(dateFormat),
    ...
    const focusOnDateCalendar = () => {
        ...
        const totalDaysInMonth = getTotalDaysInMonth.getTotalDaysInMonth(state.displayDate);
    ...
    const BodyContent = () => {
        ...
        if (view === 'month')
            return (React__namespace.createElement(MonthView, { selectedDate: state.selectedDate, displayDate: state.displayDate, onClickMonth: onClickMonth, show: showCalendar, monthRefs: monthRefs, onChangeMonth: onChangeMonth, handleTabPressOnCalendarBody: handleTabPressOnCalendarBody }));
        if (view === 'year')
            return (React__namespace.createElement(YearView, { selectedDate: state.selectedDate, displayDate: state.displayDate, onClickYear: onClickYear, show: showCalendar, yearRefs: yearRefs, onChangeMonth: onChangeMonth, handleTabPressOnCalendarBody: handleTabPressOnCalendarBody }));
        return (React__namespace.createElement(Calendar, { selectedDate: state.selectedDate, displayDate: state.displayDate, changeDate: isRange ? onChangeDateRange : onChangeDateSingle, minDate: props.minDate, maxDate: props.maxDate, mode: mode, show: showCalendar, dayRefs: dayRefs, onChangeMonth: onChangeMonth, handleTabPressOnCalendarBody: handleTabPressOnCalendarBody }));
    };
    const enterDateSingle = (event) => {
        ...
        const parsedDate = dayjs__default["default"](enteredDate, dateFormat).toDate();
    ...
    const enterDateRange = (event) => {
        ...
        const dateStart = dayjs__default["default"](start, dateFormat).toDate();
        const dateEnd = dayjs__default["default"](end, dateFormat).toDate();
        ...
        if (isValidDate(start, dateFormat) &&
            ...
            const { start: dateStart, end: dateEnd, inputDate, } = arrangeDateRange(dayjs__default["default"](start, dateFormat).toDate(), dayjs__default["default"](end, dateFormat).toDate(), dateFormat);
            setState((prevState) => ({
                ...prevState,
                inputDate: inputDate,
                selectedDate: {
    ...
    const validateDateInput = () => {
        ...
        const dateRangeValidation = () => {
            ...
            if (isValidDate(start, dateFormat) &&
                dayjs__default["default"](start, dateFormat).toDate().getFullYear() < 1900) {
                showError();
                return;
            }
            if (isValidDate(end, dateFormat) &&
                dayjs__default["default"](end, dateFormat).toDate().getFullYear() < 1900) {
                showError();
                return;
            }
        ...
        const dateSingleValidation = () => {
            const beforeMinDate = props.minDate
                ? setTimeToNoon(dayjs__default["default"](state.inputDate, dateFormat).toDate()) <
                    setTimeToNoon(new Date(props.minDate))
                : false;
            ...
            if (isValidDate(state.inputDate, dateFormat) &&
                dayjs__default["default"](state.inputDate, dateFormat).toDate().getFullYear() < 1900) {
                showError();
                return;
            }
    ...
    React__namespace.useEffect(() => {
        setDatepickerMenuId(generateId__default["default"]('datepicker', 'ul'));
    }, []);
    React__namespace.useEffect(() => {
        if (!showCalendar) {
    ...
    return (React__namespace.createElement(context.Provider, { value: contextValue },
        React__namespace.createElement(Dropdown$1, { drop: calendarPlacement, className: "input-group", onToggle: toggleDatepickerHandler },
            React__namespace.createElement(DateInput, { ref: ref, className: props.className, required: props.required, placeholder: props.placeholder, disabled: props.disabled, id: props.id, isRange: isRange, inputDate: state.inputDate, isInvalid: state.invalid, dateFormat: dateFormat, validateDateInput: validateDateInput, enterDateRange: enterDateRange, enterDateSingle: enterDateSingle, "aria-invalid": state.invalid, "aria-describedby": state.invalid ? feedbackId : '' }),
            React__namespace.createElement(Dropdown$1.Toggle, { ref: dropdownToggleRef, className: "rounded-0 border", variant: "outline-dark", disabled: props.disabled, "aria-label": "Open Datepicker", "aria-haspopup": "dialog", "aria-controls": datepickerMenuId },
                React__namespace.createElement("i", { className: "bi bi-calendar" })),
            React__namespace.createElement(Button, { onClick: clear, disabled: props.disabled, variant: clearBtnVariant, "aria-label": "Reset Datepicker" },
                React__namespace.createElement("i", { className: "bi bi-x" }),
                React__namespace.createElement("span", { className: "visually-hidden" }, "clear")),
            React__namespace.createElement(FormControl$1.Feedback, { type: "invalid", id: feedbackId }, props.invalidFeedback),
            React__namespace.createElement(Dropdown$1.Menu, { id: datepickerMenuId, className: "sgds datepicker", as: "div", role: "dialog", "aria-modal": "true", "aria-label": ariaLabelsForMenu[view] },
