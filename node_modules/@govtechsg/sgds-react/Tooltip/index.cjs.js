...
const TooltipBox = React__namespace.forwardRef(({ bsPrefix, placement, className, style, children, popper: _, show: _2, closeBtn, ...props }, ref) => {
    ...
    return (React__namespace.createElement(SGDSWrapper, { ref: ref, style: style, role: "tooltip", "x-placement": primaryPlacement, className: classNames__default["default"](className, bsPrefix, `bs-tooltip-${bsDirection}`), ...props },
        React__namespace.createElement("div", { className: `${bsPrefix}-inner` }, children)));
});
TooltipBox.defaultProps = defaultProps$3;
TooltipBox.displayName = 'Tooltip';
TooltipBox.propTypes = propTypes$5;
...
const Popover = React__namespace.forwardRef(({ bsPrefix, placement, className, style, children, body, popper: _, show: _1, ...props }, ref) => {
...
function wrapRefs(props) {
    const { ref } = props;
    props.ref = ref.__wrapped || (ref.__wrapped = (r) => ref(safeFindDOMNode__default["default"](r)));
}
const Overlay = React__namespace.forwardRef(({ children: overlay, transition, popperConfig = {}, rootClose = false, ...outerProps }, outerRef) => {
    const popperRef = React.useRef({});
    const [ref, modifiers] = useOverlayOffset(outerProps.offset);
    const mergedRef = useMergedRefs__default["default"](outerRef, ref);
    const actualTransition = transition === true ? Fade : transition || undefined;
    return (React__namespace.createElement(BaseOverlay__default["default"], { ...outerProps, rootClose: rootClose, ref: mergedRef, popperConfig: {
            ...popperConfig,
            modifiers: modifiers.concat(popperConfig.modifiers || []),
        }, 
        //@ts-ignore
        transition: actualTransition }, (overlayProps, { popper: popperObj, show }) => {
...
function OverlayTrigger({ trigger = ['hover', 'focus'], overlay, children, popperConfig = {}, show: propsShow, defaultShow = false, onToggle, delay: propsDelay, placement, flip = placement && placement.indexOf('auto') !== -1, ...props }) {
    const triggerNodeRef = React.useRef(null);
    const mergedRef = useMergedRefs__default["default"](triggerNodeRef, utils.getChildRef(children));
...
OverlayTrigger.propTypes = propTypes$1;
...
const Tooltip = ({ placement = 'top', type = 'hover', content = '', children, ...props }) => {
    const [show, setShow] = React.useState(false);
    const target = React.useRef(null);
    const [tooltipId, setTooltipId] = React.useState('');
    React__namespace.useEffect(() => {
        setTooltipId(generateId__default["default"]('tooltip', 'div'));
    }, []);
    const clickToolTip = () => (React__namespace.createElement(React__namespace.Fragment, null,
        ...
        React__namespace.createElement(Overlay, { target: target.current, show: show, placement: placement, rootClose: true, onHide: () => setShow(false) }, (props) => (React__namespace.createElement(TooltipBox, { ...props, id: tooltipId }, content)))));
